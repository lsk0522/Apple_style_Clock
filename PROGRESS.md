...
| | |
|---|---|
| **진행 중 Phase** | **Phase 9 까지 구현 완료** — v0.1 MVP 범위 도달, 실기기 검증 대기 |
| **마지막 갱신** | 2026-09-20 |
| **마지막 커밋** | feat(donate): Play Billing 연결 |
...
1. **[사용자]** **실기기 검증** — 아래 "미검증" 목록 전부
2. **Phase 9 잔여** — **Play Console 에 상품 3개 등록 필요.**
   `donate_coffee` / `donate_meal` / `donate_generous` (일회성 상품).
   등록 전까지, 그리고 Play 가 설치하지 않은 빌드에서는 후원 목록이 비는 것이 정상이다
3. **Phase 10** — 다국어. 현재 `values/` 전부 한국어라 플레이 배포 시 걸린다
4. **Phase 6 잔여** — 위젯 미리보기 · 드래그 정렬
5. **Phase 4 잔여** — 월드 시계 도시 선택 (지금은 뉴욕/런던/도쿄 하드코딩)
6. **Phase 7 잔여** — 자동 밝기 (조도 센서는 야간 모드용으로 이미 붙어 있다)
7. **Phase 11** — 릴리스 서명키 · Play 스토어 등재
