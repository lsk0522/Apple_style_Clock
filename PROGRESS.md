...
| **마지막 갱신** | 2026-09-20 |
| **마지막 커밋** | fix(main): 탭 전환에서 해석되지 않는 using 호출 제거 |
...
1. **[사용자]** **실기기 검증** — 삼성 기본 위젯이 실제로 붙는지.
   이게 이 앱의 차별점이고, 일부 위젯은 런처 호스트에서만 정상 동작할 수 있다
2. **Phase 3 잔여** — `DreamService` 보조 경로 (오버레이 권한 거부한 사용자용).
   CLAUDE.md 가 "둘 다 구현한다"고 못박은 항목인데 코드에 없다
3. **Phase 8** — 개발자 모드: 지금 5개 행 전부 비활성, 실제 진단 미연결
4. **Phase 9** — 후원: Play Billing 미연결, 금액은 자리표시자
5. **Phase 10** — 다국어. 현재 `values/` 4개가 전부 한국어라 플레이 배포 시 걸린다
6. **Phase 6 잔여** — 위젯 미리보기 · 드래그 정렬
7. **Phase 4 잔여** — 월드 시계 도시 선택 (지금은 뉴욕/런던/도쿄 하드코딩)
8. **Phase 7 잔여** — 자동 밝기 (조도 센서는 야간 모드용으로 이미 붙어 있다)
