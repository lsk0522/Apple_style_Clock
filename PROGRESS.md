...
| **마지막 갱신** | 2026-09-21 |
| **마지막 커밋** | build(release): 릴리스 서명·R8 규칙 |
...
1. **[사용자]** **실기기 검증** — 빌드 #32 이후 전부 미검증. 아래 검증 목록 참고.
   먼저 확인하지 않으면 그 위에 쌓는 것이 전부 추측이 된다
...
4. 크래시 리포팅 (배포 후엔 이게 없으면 사용자 문제를 못 본다)
5. 배터리 프로파일링 — 목표 "StandBy 1시간당 5% 이하" 미측정
6. 폴더블 · 태블릿 레이아웃
7. 일본어
8. 위젯 미리보기 갤러리 · 세그먼트 컨트롤 · 슬라이더 · 모달 시트
