...
2. **전원 브로드캐스트를 매니페스트에 등록하지 말 것.**
   `ACTION_POWER_CONNECTED` 는 암시적 브로드캐스트 예외 목록에 **없어서**
   Android 8+ 에서 리시버가 절대 깨어나지 않는다. 충전 감지는
   `JobScheduler.setRequiresCharging` + 충전 중에만 도는 포그라운드 서비스로 한다.
   (`BOOT_COMPLETED` 는 예외 목록에 있어서 매니페스트 등록이 동작한다)
3. **위젯은 자체 제작이 아니라 `AppWidgetHost` 기반 실제 시스템 위젯 호스팅**이 핵심 요구사항. 갤럭시 기본 위젯이 들어가야 한다.
4. **SF Pro 폰트는 번들 불가** (Apple 플랫폼 전용 라이선스). 실제 번들은 `Pretendard` 가변 폰트 하나(OFL).
5. **Apple 상표/에셋 사용 금지** — 아이콘은 직접 제작 또는 오픈소스.
6. **삼성 절전 정책**이 서비스를 죽인다 — 배터리 최적화 예외 온보딩이 필수.
7. ~~One UI Daily Board 와 충돌~~ — **사실이 아니었다.** Daily Board 는 갤럭시
