...
```
설정 → 애플리케이션 → 특별한 접근 권한 → 다른 앱 위에 표시 → Apple Style Clock → 허용
```
...
```
설정 → 배터리 → 백그라운드 사용 제한 → 사용 안 함 앱 절전 → Apple Style Clock 제외
```
...
| 제조사 | 경로 |
|---|---|
| **삼성 (One UI)** | 설정 → 배터리 → 백그라운드 사용 제한 → **절전 모드로 전환되지 않는 앱**에 추가 |
