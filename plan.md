...
| 최소 지원 | Android 10 (API 29) — 아래 §10 Q2 확인 필요 |
| 타깃 | API 36 |
...
| # | 질문 | 선택지 / 추천 |
|---|---|---|
| **Q1** | 배포용 앱 이름 | 상표 안전 필요. 추천: **Nightstand**, **Halo Clock**, **Dock Clock**, **Standby Clock** (저장소 이름은 그대로 두어도 무방) |
| **Q2** | 최소 지원 버전 | **API 29(Android 10) 추천** — 점유율 약 95%, 레거시 분기 대폭 감소. API 26으로 낮추면 개발 시간 +15% |
| **Q3** | 날씨 위젯 데이터 | 삼성 위젯을 호스팅하면 자체 날씨는 불필요할 수도. **자체 날씨는 v1.1로 연기 추천** |
| **Q4** | 후원 방식 | **Play 인앱결제(정석)** — 외부 링크는 정책 위반 소지 |
| **Q5** | 개발자 모드 공개 범위 | **항상 노출** (요청하신 5탭 구성 그대로) |
| **Q6** | 1차 목표 범위 | **Phase 0~5까지를 v0.1 MVP**로 잡고 먼저 폰에 설치해보기 추천 |
