...
### Phase 7 — 야간 모드 · 센서 · 번인 방지 · 2~3일 ✅ 구현 완료
...
### Phase 9 — 후원 (Play Billing) · 2~3일 🚧 일부
...
> Phase 0~11의 **코드**는 사실상 다 들어갔습니다. 여기서부터는 성격이 다릅니다:
> 남은 일의 절반은 제가 할 수 없는 일(키 생성, 플레이 콘솔, 실기기 확인)이고,
> 나머지 절반은 **검증 없이는 손대면 안 되는 일**입니다.

### 8-1. 지금 바로 막고 있는 것 — 실기기 검증

빌드 #32 이후로 폰에서 확인된 것은 위젯 렌더링뿐입니다. 그 사이 20개가 넘는
빌드가 나갔고, 전부 미검증입니다. **먼저 확인하지 않으면 그 위에 쌓는 것이
전부 추측이 됩니다.** PROGRESS.md의 검증 목록을 순서대로 비우는 것이 다음 할 일.

### 8-2. 내가 할 수 있는, 남은 구현

| | 왜 |
|---|---|
| 크래시 리포팅 | 배포 후에는 이게 없으면 사용자 문제를 영원히 못 봅니다 |
| 배터리 프로파일링 | 목표 "1시간당 5% 이하"가 실제로 지켜지는지 미측정 |
| 폴더블 · 태블릿 레이아웃 | StandBy는 가로 화면 전제인데 접는 기기에서 미확인 |
| 일본어 | 한국어·영어는 끝. 일본어는 시장 크기 대비 비용이 낮습니다 |
| 위젯 미리보기 갤러리 | 현재 피커는 목록. iOS는 큰 미리보기 격자 |
| 세그먼트 컨트롤 · 슬라이더 · 모달 시트 | 디자인 시스템에서 아직 비어 있는 칸 |

### 8-3. 사용자만 할 수 있는 것

| | 주의 |
|---|---|
| **릴리스 키 생성** | `keystore.properties.example` 참고. **이 키를 잃으면 자기 앱을 영원히 업데이트할 수 없습니다.** 10년 뒤에도 가지고 있을 곳에 백업 |
| GitHub Secrets 등록 | `NIGHTSTAND_KEYSTORE` / `_STORE_PASSWORD` / `_KEY_ALIAS` / `_KEY_PASSWORD` |
| **Play Console 상품 3개** | `donate_coffee` · `donate_meal` · `donate_generous` (일회성). 등록 전에는 후원 탭이 비는 것이 정상 |
| 개인정보처리방침 | GitHub Pages로 충분. 플레이 필수 |
| 스토어 등재물 | 스크린샷·설명·아이콘 |
| 테스트 트랙 | 신규 개발자 계정은 **비공개 테스트 12명 · 14일**을 통과해야 프로덕션에 올릴 수 있습니다. 여기가 가장 긴 구간 |
...
2. 검증에서 나온 버그 수정
3. 크래시 리포팅 + 배터리 측정
4. 릴리스 키 · Play Console 준비 (사용자)
5. 내부 테스트 트랙에 AAB 업로드
6. 비공개 테스트 12명 · 14일
7. 프로덕션

> 3번과 4번은 병렬입니다. 4번의 14일 대기가 전체 일정을 지배하므로,
> **키와 콘솔 준비를 먼저 시작하는 편이 빠릅니다.**
